...
1. GitHub Action מתוזמן רץ פעם בשעה (`0 * * * *`). הג'וב הזה מריץ לולאה פנימית שסורקת כל 30 דקות במשך ~55 דקות — מתי שצריך GitHub יהיה בעומס ולא יבטיח לי שעה מדויקת בקרון, ואני לא רוצה שהמרווח בפועל בין סריקות יקפוץ ל-2-4 שעות.
...
5. כל מודעה שלא נראתה לפני כן נחשבת חדשה ומתווספת ל-`state/seen-ads.json`. ה-scan הוא אדיטיבי בלבד — הוא **לא מוחק** מודעות.
6. אם יש מודעות חדשות, נשלחת הודעת Telegram אחת מרוכזת עם כל הקישורים (או email ללב הפארק).
7. אחרי כל איטרציה ה־Action דוחף בחזרה לענף `state` את הקבצים המעודכנים.
...
4. אחרי שהקוד נדחף, להיכנס ל-`Actions` → `Yad2 Scan` → `Run workflow` כדי להפעיל ידנית פעם ראשונה. מהריצה הבאה והלאה ה-Action יתוזמן כל שעה, יסרוק בתוכו כל 30 דקות.
...
- `LOOP_INTERVAL_MS` — מרווח בין איטרציות בתוך הג'וב השעתי של scan. ברירת מחדל `1800000` (30 דקות).
- `LOOP_BUDGET_MS` — תקציב wall-clock כולל לג'וב לפני שהלולאה יוצאת. ברירת מחדל `3300000` (55 דקות) — קצת פחות משעה כדי שלא נתנגש בקרון הבא.
- `LOOP_PERSIST_EACH_ITERATION` — אם `true` (ברירת מחדל), דוחפים את ה-state אחרי כל איטרציה כך שקריסה באמצע הג'וב לא תאבד מודעות שנמצאו.
