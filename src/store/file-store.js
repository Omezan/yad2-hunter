...
function commitAds({ newAds = [], existingAds = [] }) {
  // The scan is *additive only*. We never delete here, because a
  // partial scrape (anti-bot, captcha, timeout) would otherwise wipe
  // legitimate listings out of `seen-ads.json` and the next run would
  // wrongly notify them as "new". Deletions are the exclusive job of
  // the health-check, which probes every suspect URL with a 404 check
  // before removing it.
  const seen = loadSeenAds();
